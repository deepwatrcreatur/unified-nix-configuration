    ...
    in
    {
      ...
      systemd.services.nix-attic-token = {
        ...
        script = ''
          ...
          token_file="${tokenFilePath}"
          if [[ ! -f "$token_file" ]]; then
            echo "Warning: Attic client token not found. Nix pulls will likely fail." >&2
            exit 0
          fi

          if [[ ! -r "$token_file" ]]; then
            echo "Warning: Attic client token not readable. Nix pulls will likely fail." >&2
            exit 0
          fi

          echo "Preparing Attic token for Nix daemon..." >&2
          mkdir -p /run/nix
