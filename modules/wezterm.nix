...
let
  ...
  # Helper function to format mouse event
  formatMouseEvent = binding: 
    if binding.event != "" then
      "{ event = { ${binding.event} }, mods = '${binding.mods}', action = wezterm.action.${binding.action} }"
    else
      "{ mods = '${binding.mods}', action = wezterm.action.${binding.action} }";
  ...
  defaultConfig = ''
    ...
    config.mouse_bindings = {
      ${concatStringsSep ",\n      " (map formatMouseEvent cfg.mouseBindings)}
    }
    ''}
