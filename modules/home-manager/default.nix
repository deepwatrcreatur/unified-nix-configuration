...
in
{
  ...
  # Some configs are intentionally materialized as mutable regular files during
  # activation. Rotate stale Home Manager backups out of the way so repeated
  # activations do not fail when using the fixed ".bak" extension.
  home.activation.rotateManagedBackups = lib.hm.dag.entryBefore [ "checkLinkTargets" ] ''
    rotate_backup() {
      local path="$1"
      local rotated="''${path}.previous"

      if [[ -e "$path" ]]; then
        $DRY_RUN_CMD rm -f "$rotated"
        $DRY_RUN_CMD mv "$path" "$rotated"
      fi
    }
