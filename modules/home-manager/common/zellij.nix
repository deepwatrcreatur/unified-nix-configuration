...
let
  ...
  configFile = pkgs.writeText "zellij-config.kdl" ''
    ...
    // Additional custom{ config, lib, pkgs, ... }:
...
in {
  options.programs.zellij-extended = {
    ...
    enableClipboardTools = mkOption {
      type = types.bool;
      default = false;
      description = "Install clipboard tools (wl-clipboard, xclip). Enable for local Wayland/X11 systems.";
    };
      type = types.bool;
      default = false;
