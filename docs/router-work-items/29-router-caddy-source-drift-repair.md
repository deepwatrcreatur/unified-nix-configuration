...
- Identified that `github.com/caddy-dns/cloudflare` had a new tag `v0.2.4` (March 23, 2026)
  which was required for compatibility with new Cloudflare token formats.
- Updated `caddy.nix` to use `v0.2.4` for the cloudflare plugin.
- Regenerated and verified the fixed-output hash for Caddy `2.11.2` with updated plugins.
- Rebuilds now pass `installCheck` and version verification.
