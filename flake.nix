{
  description = "Unified NixOS configuration with Home Assistant module";
  ...
  outputs = { self, nixpkgs, sops-nix, home-manager, ... }:
    let
      ...
      # Helper to import all .nix files from a directory
      importModules = path:
        let
      ...
      # Path to the homeserver host directory
      homeserverPath = ./hosts/homeserver;
    in {
      nixosConfigurations.homeserver = lib.nixosSystem {
        ...
        modules =
          ...
          ++ (importModules (homeserverPath + "/modules"))
          ++ [
            ({ config, pkgs, lib, ... }: {
              # === SOPS Configuration ===
              ...
              home-manager = {
                ...
                users.deepwatrcreatur = import (homeserverPath + "/users/deepwatrcreatur.nix");
                users.root = import (homeserverPath + "/users/root.nix");
              };
              systemd.services."home-assistant".wants = [ "sops-nix.service" ];
              systemd.services."home-assistant".after = [ "sops-nix.service" ];
            })
            # Import the Home Assistant config from the host-specific file
            (import (homeserverPath + "/modules/home-assistant.nix"))
          ];
      };
    };
}
